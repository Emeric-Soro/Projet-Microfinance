...
(function () {
  ...
  function setSession(data) {
    ...
    if (data.utilisateur)
      localStorage.setItem("sf_user", JSON.stringify(data.utilisateur));
  ...
  async function apiFetch(path, options) {
    ...
    try {
      ...
      var msg =
        "Impossible de joindre le serveur (localhost:8080). Vérifiez que le backend est démarré.";
      if (
        fetchErr &&
        fetchErr.message &&
        ...
        msg =
          "Erreur CORS : le backend refuse les requêtes depuis cette origine.";
      }
      throw new Error(msg);
    }
    // 401 → session expirée
    if (res.status === 401) {
      clearSession();
      var pg = (window.location.pathname.split("/").pop() || "").replace(
        /\.html$/i,
        "",
      );
    ...
    if (res.status === 403) {
      if (typeof window.showToast === "function")
        window.showToast("Accès refusé : droits insuffisants.", "error");
      return res;
    }
    return res;
  }
  ...
  var Auth = {
    ...
    logout: async function () {
      if (!(await showConfirm("Déconnexion", "Êtes-vous sûr de vouloir vous déconnecter ?", { icon: "warning", confirmText: "Se déconnecter", confirmClass: "btn-danger" }))) {
        return;
      }
      var token = getToken();
      if (token) {
  ...
  function guardSession() {
    var currentPage = (window.location.pathname.split("/").pop() || "").replace(
      /\.html$/i,
      "",
    );
  ...
  var Clients = {
    ...
    modifierStatut: async function (idClient, nouveauStatut) {
      return await apiFetch(
        "/api/v1/clients/" +
          ...
          encodeURIComponent(nouveauStatut),
        { method: "PUT" },
      );
    },
    ...
    mettreAJourKyc: async function (idClient, data) {
      return await apiFetch("/api/v1/clients/" + idClient + "/kyc", {
        method: "PUT",
        body: JSON.stringify(data),
      });
    },
  ...
  var Comptes = {
    ...
    obtenir: async function (numCompte) {
      return await apiFetch("/api/v1/comptes/" + numCompte);
    },
    ouvrir: async function (
      ...
      if (idAgence) body.idAgence = idAgence;
      if (decouvertAutorise != null) body.decouvertAutorise = decouvertAutorise;
      return await apiFetch("/api/v1/comptes", {
        method: "POST",
        body: JSON.stringify(body),
      });
    },
    bloquer: async function (numCompte, motif) {
      return await apiFetch("/api/v1/comptes/" + numCompte + "/blocage", {
        method: "PUT",
        body: JSON.stringify({ motif: motif }),
      });
    },
    debloquer: async function (numCompte, motif) {
      return await apiFetch("/api/v1/comptes/" + numCompte + "/deblocage", {
        method: "PUT",
        body: JSON.stringify({ motif: motif }),
      });
    },
    cloturer: async function (numCompte, motif) {
      return await apiFetch("/api/v1/comptes/" + numCompte + "/cloture", {
        method: "PUT",
        body: JSON.stringify({ motif: motif }),
      });
    },
    modifierDecouvert: async function (numCompte, nouveauPlafond) {
      return await apiFetch("/api/v1/comptes/decouvert", {
        method: "PUT",
        body: JSON.stringify({
          numCompte: numCompte,
          nouveauPlafond: nouveauPlafond,
        }),
      });
    },
    listerClientComptes: async function (idClient, page, size) {
      return await apiFetch(
        "/api/v1/comptes/client/" +
          idClient +
          "?page=" +
          (page || 0) +
          "&size=" +
  ...
  var Cartes = {
    lister: async function (numCompte, page, size) {
      let path =
        "/api/v1/cartes-visa?page=" + (page || 0) + "&size=" + (size || 20);
      if (numCompte) path += "&numCompte=" + encodeURIComponent(numCompte);
      return await apiFetch(path);
    },
    commander: async function (numCompte) {
      return await apiFetch(
        "/api/v1/cartes-visa?numCompte=" + encodeURIComponent(numCompte),
        { method: "POST" },
      );
    },
    opposer: async function (numCarte) {
      return await apiFetch("/api/v1/cartes-visa/" + numCarte + "/opposition", {
        method: "PUT",
      });
    },
    obtenir: async function (numeroCarte) {
      return await apiFetch("/api/v1/cartes-visa/" + numeroCarte);
    },
    opposerParId: async function (idCarte) {
      return await apiFetch(
        "/api/v1/cartes-visa/id/" + idCarte + "/opposition",
        { method: "PUT" },
      );
    },
  };
  ...
  var Beneficiaires = {
    ...
    creer: async function (clientId, data) {
      return await apiFetch("/api/v1/beneficiaires?clientId=" + clientId, {
        method: "POST",
        body: JSON.stringify(data),
      });
    },
    modifier: async function (id, clientId, data) {
      return await apiFetch(
        "/api/v1/beneficiaires/" + id + "?clientId=" + clientId,
  ...
  var Transactions = {
    ...
    depotInitial: async function (numCompte, montant, idInitiateur) {
      return await apiFetch(
        "/api/v1/comptes/" + encodeURIComponent(numCompte) + "/depot-initial",
        {
          method: "POST",
          body: JSON.stringify({
            montant: montant,
    ...
    historiqueCompte: async function (numCompte, page, size) {
      return await apiFetch(
        "/api/v1/transactions/comptes/" +
          encodeURIComponent(numCompte) +
          "/historique?page=" +
          (page || 0) +
          "&size=" +
          (size || 20),
    ...
    recu: async function (referenceUnique) {
      return await apiFetch(
        "/api/v1/transactions/" + encodeURIComponent(referenceUnique) + "/recu",
      );
    },
    /** POST /api/v1/transactions/virement */
    virement: async function (
      compteSource,
      compteDestination,
      montant,
      idGuichetier,
      ...
      return await apiFetch(
        "/api/v1/transactions/virement?idGuichetier=" +
          encodeURIComponent(idGuichetier),
        {
          method: "POST",
          body: JSON.stringify({
    ...
    listerEnAttente: async function (page, size) {
      return await apiFetch(
        "/api/v1/transactions/en-attente?page=" +
          (page || 0) +
          "&size=" +
          (size || 20),
      );
    ...
    approuver: async function (referenceUnique, idSuperviseur) {
      return await apiFetch(
        "/api/v1/transactions/" +
          encodeURIComponent(referenceUnique) +
          "/approbation",
        {
          method: "PUT",
    ...
    listerToutes: async function (page, size) {
      return await apiFetch(
        "/api/v1/transactions?page=" + (page || 0) + "&size=" + (size || 20),
      );
    },
  };
  ...
  const Conformite = {
    ...
    alertesLcbFt: async function (page = 0, size = 10) {
      return await apiFetch(
        "/api/v1/conformite/alertes-lcbft?page=" + page + "&size=" + size,
      );
    },
    /** GET /api/v1/conformite/kyc/expires */
    kycExpires: async function (page = 0, size = 10) {
      return await apiFetch(
        "/api/v1/conformite/kyc/expires?page=" + page + "&size=" + size,
      );
    },
  };
  ...
  const Securite = {
    ...
    modifierRole: async function (id, data) {
      return await apiFetch("/api/v1/securite/roles/" + id, {
        method: "PUT",
        body: JSON.stringify(data),
      });
    },
    ...
    mettreAJourPermissions: async function (data) {
      return await apiFetch("/api/v1/securite/permissions", {
        method: "PUT",
        body: JSON.stringify(data),
      });
    },
    ...
    creerUtilisateur: async function (data) {
      return await apiFetch("/api/v1/securite/utilisateurs", {
        method: "POST",
        body: JSON.stringify(data),
      });
    },
    modifierUtilisateur: async function (id, data) {
      return await apiFetch("/api/v1/securite/utilisateurs/" + id, {
        method: "PUT",
        body: JSON.stringify(data),
      });
    },
    activerDesactiverUtilisateur: async function (id, actif) {
      return await apiFetch(
        "/api/v1/securite/utilisateurs/" + id + "/activer-desactiver",
        {
          method: "PUT",
  ...
  var Agences = {
    lister: async function (page = 0, size = 50) {
      return await apiFetch(
        "/api/v1/parametrages/agences?page=" + page + "&size=" + size,
      );
    },
    /** GET /api/v1/parametrages/agences/actives — accessible à tous les rôles terrain */
    actives: async function () {
      return await apiFetch("/api/v1/parametrages/agences/actives");
    },
    creer: async function (data) {
      return await apiFetch("/api/v1/parametrages/agences", {
        method: "POST",
        body: JSON.stringify(data),
      });
    },
    ...
    desactiver: async function (id) {
      return await apiFetch(
        "/api/v1/parametrages/agences/" + id + "/desactiver",
        { method: "PUT" },
      );
    },
  };
  ...
  async function downloadFile(path, defaultFilename) {
    try {
      if (typeof window.showToast === "function")
        window.showToast("Téléchargement du fichier en cours...", "info");
      ...
      if (res && res.ok) {
        ...
        if (typeof window.showToast === "function")
          window.showToast("Téléchargement terminé.", "success");
      } else if (res) {
        ...
        if (typeof window.showToast === "function")
          window.showToast(
            "Erreur : " +
              (err.message || "Impossible de télécharger le fichier."),
            "error",
          );
      }
    } catch (e) {
  ...
  var Credits = {
    ...
    simuler: async function (montant, taux, duree, methode) {
      return await apiFetch("/api/v1/credits/simulation", {
        ...
        body: JSON.stringify({
          ...
          taux: taux,
          duree: duree,
          methode: methode || "DEGRESSIF",
        }),
      });
    },
    /** POST /api/v1/credits/demandes */
    soumettreDemande: async function (data) {
      return await apiFetch("/api/v1/credits/demandes", {
        method: "POST",
        body: JSON.stringify(data),
      });
    },
    ...
    consulterDemande: async function (idDemande) {
      return await apiFetch("/api/v1/credits/demandes/" + idDemande);
    },
    /** PUT /api/v1/credits/demandes/:id/decision */
    deciderDemande: async function (idDemande, decision, motifRejet) {
      return await apiFetch(
        "/api/v1/credits/demandes/" + idDemande + "/decision",
        {
          method: "PUT",
          body: JSON.stringify({
            idDemande: idDemande,
    ...
    consulter: async function (idCredit) {
      return await apiFetch("/api/v1/credits/" + idCredit);
    },
    /** GET /api/v1/credits */
    lister: async function (statut, idClient, page, size) {
      var url = "/api/v1/credits?page=" + (page || 0) + "&size=" + (size || 20);
      if (statut) url += "&statut=" + encodeURIComponent(statut);
      if (idClient) url += "&idClient=" + idClient;
      return await apiFetch(url);
    },
    /** GET /api/v1/credits/client/:idClient */
    listerClient: async function (idClient, page, size) {
      return await apiFetch(
        "/api/v1/credits/client/" +
          idClient +
          "?page=" +
          (page || 0) +
          "&size=" +
    ...
    approuver: async function (idCredit) {
      return await apiFetch("/api/v1/credits/" + idCredit + "/approuver", {
        method: "POST",
      });
    },
    ...
    instruire: async function (idCredit) {
      return await apiFetch("/api/v1/credits/" + idCredit + "/instruire", {
        method: "POST",
      });
    },
    /** POST /api/v1/credits/:id/restructurer */
    restructurer: async function (idCredit, nouvelleDureeMois, nouveauTaux) {
      var url =
        "/api/v1/credits/" +
        idCredit +
        "/restructurer?nouvelleDureeMois=" +
        nouvelleDureeMois;
      if (nouveauTaux != null) url += "&nouveauTaux=" + nouveauTaux;
      return await apiFetch(url, { method: "POST" });
    },
    /** POST /api/v1/credits/:id/passation */
    passerSouffrance: async function (idCredit) {
      return await apiFetch("/api/v1/credits/" + idCredit + "/passation", {
        method: "POST",
      });
    },
  ...
  var ProduitsCredit = {
    ...
    lister: async function (p, s) {
      return await apiFetch(
        "/api/v1/parametrages/produits-credit?page=" +
          (p || 0) +
          "&size=" +
          (s || 50),
      );
    },
    consulter: async function (id) {
      return await apiFetch("/api/v1/parametrages/produits-credit/" + id);
    },
    creer: async function (data) {
      return await apiFetch("/api/v1/parametrages/produits-credit", {
        method: "POST",
        body: JSON.stringify(data),
      });
    },
  ...
  var ProduitsEpargne = {
    ...
    lister: async function (p, s) {
      return await apiFetch(
        "/api/v1/parametrages/produits-epargne?page=" +
          (p || 0) +
          "&size=" +
          (s || 50),
      );
    ...
    consulter: async function (id) {
      return await apiFetch("/api/v1/parametrages/produits-epargne/" + id);
    },
    creer: async function (data) {
      return await apiFetch("/api/v1/parametrages/produits-epargne", {
        method: "POST",
        body: JSON.stringify(data),
      });
    },
  ...
  var AuditLogs = {
    lister: async function (filters = {}, page = 0, size = 50) {
      var queryParams = new URLSearchParams();
      queryParams.append("page", page);
      queryParams.append("size", size);
      if (filters && typeof filters === "object") {
        if (filters.utilisateur) queryParams.append("utilisateur", filters.utilisateur);
        if (filters.action) queryParams.append("action", filters.action);
        if (filters.ressource) queryParams.append("ressource", filters.ressource);
        if (filters.idEntite) queryParams.append("idEntite", filters.idEntite);
        if (filters.du) queryParams.append("du", filters.du);
        if (filters.au) queryParams.append("au", filters.au);
        if (filters.statut) queryParams.append("statut", filters.statut);
      }
      return await apiFetch("/api/v1/audit-logs?" + queryParams.toString());
    }
  };
  ...
  var Notifications = {
    ...
    marquerLue: async function (id, idClient) {
      if (!idClient) {
        var user = getUser();
        idClient = (user && user.clientId) || new URLSearchParams(window.location.search).get('clientId') || 1;
      }
      return await apiFetch("/api/v1/notifications/" + id + "/lu?idClient=" + idClient, {
        method: "PUT"
      });
    },
  ...
  var Tarification = {
    refreshCache: async function () {
      return await apiFetch("/api/v1/tarification/parametres/cache/refresh", {
        method: "POST"
      });
    }
  };
...
(function () {
  ...
  const icons = {
    ...
    eyeOff:
      '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M17.94 17.94A10.94 10.94 0 0 1 12 20C5 20 1 12 1 12a20.29 20.29 0 0 1 5.06-5.94"/><path d="M9.9 4.24A10.45 10.45 0 0 1 12 4c7 0 11 8 11 8a20.55 20.55 0 0 1-3.06 4.44"/><line x1="1" y1="1" x2="23" y2="23"/></svg>',
  };
  ...
  const navGroups = [
      ...
      items: [
        {
          ...
          label: "Liste clients",
          icon: icons.users,
          match: ["clients.html", "client-detail.html"],
        },
        {
          href: "client-create.html",
          label: "Nouveau client",
          icon: icons.users,
          match: ["client-create.html"],
        },
        {
          href: "kyc-validation.html",
          label: "Validation KYC",
          icon: icons.audit,
          match: ["kyc-validation.html"],
        },
        {
          href: "blacklist-client.html",
          label: "Blacklist",
          icon: icons.audit,
          match: ["blacklist-client.html"],
        },
      ],
    },
    {
      ...
      items: [
        {
          ...
          match: [
            ...
            "detail-compte.html",
            "blocage-compte.html",
            "fermeture-compte.html",
          ],
        },
        {
          href: "ouverture-compte.html",
          label: "Ouverture de compte",
          icon: icons.card,
          match: ["ouverture-compte.html"],
        },
      ],
    },
    {
      ...
      items: [
        ...
        },
        {
          ...
          label: "Validation 4-eyes",
          icon: icons.audit,
          match: ["validation.html"],
        },
        {
    ...
    },
    {
      ...
      items: [
        {
          href: "credit-simulation.html",
          label: "Simulation",
          icon: icons.credit,
          match: ["credit-simulation.html"],
        },
        {
          href: "credit-demandes.html",
          label: "Demandes",
          icon: icons.credit,
          match: ["credit-demandes.html"],
        },
        {
          href: "credits.html",
          label: "Crédits actifs",
          icon: icons.credit,
          match: ["credits.html", "credit-detail.html"],
        },
        {
          href: "credit-suivi.html",
          label: "Suivi avancé",
          icon: icons.credit,
          match: ["credit-suivi.html"],
        },
      ],
    },
    {
      ...
      items: [
        {
          href: "personnel-create.html",
          label: "Ajouter personnel",
          icon: icons.users,
          match: ["personnel-create.html"],
        },
        {
          href: "agences.html",
          label: "Agences",
    ...
    if (role === "ADMIN") {
      return [
        ...
        },
        {
          ...
          href: "direction.html",
          label: "Direction",
          match: ["direction.html"],
        },
        {
          ...base,
          href: "dashboard-superviseur.html",
          label: "Supervision",
          match: ["dashboard-superviseur.html"],
        },
      ];
    }
    if (role === "DIRECTEUR") {
  ...
  function renderSidebar() {
    ...
    const roles =
      Array.isArray(user.roles) && user.roles.length ? user.roles[0] : "Agent";
  ...
  function validateStep(stepContent) {
    const requiredFields = Array.from(
      stepContent.querySelectorAll("[required]"),
    );
  ...
  function initStepper() {
    ...
    showStep(
      Number.isNaN(savedStep) ? 0 : Math.min(savedStep, steps.length - 1),
    );
    ...
    nextButtons.forEach((button) => {
      button.addEventListener("click", () => {
        ...
        if (
          ...
          !validateStep(stepContents[currentStep])
        )
          return;
    ...
    document.querySelectorAll(".export-btn").forEach((button) => {
      button.addEventListener("click", () => {
        ...
        if (label.includes("CSV")) {
          ...
          showToast(
            "Export PDF préparé côté interface. Le backend générera le document officiel.",
            "info",
          );
        }
      });
  ...
  function initOtp() {
    ...
    function syncSubmitState() {
      if (submit)
        submit.disabled = inputs.some((input) => input.value.length !== 1);
    }
    ...
    form.addEventListener("submit", async (event) => {
      ...
      try {
        ...
        const res = await window.Auth.verifierOtp(login, code);
        ...
        if (res.ok) {
          ...
          setTimeout(() => {
            window.location.href =
              window.SF && window.SF.getHomeDashboardUrl
        ...
        setTimeout(
          () =>
            form.querySelector(".otp-inputs")?.classList.remove("is-shaking"),
          320,
        );
  ...
  function initUploadZones() {
    document.querySelectorAll(".upload-zone").forEach((zone) => {
      ...
      if (!input) return;
      const preview =
        zone.querySelector(".upload-preview") || document.createElement("div");
  ...
  function renderFiles(files, preview) {
    preview.innerHTML = Array.from(files)
      .map((file) => `<span class="upload-file">${file.name}</span>`)
      .join("");
  }
  ...
  function showConfirm(title, message, options = {}) {
    ...
    return new Promise((resolve) => {
      ...
      overlay.style.background = 'rgba(8, 67, 85, 0.5)';
      ...
      overlay.innerHTML = `
        <div class="modal" style="max-width: 440px; transform: scale(0.95); transition: transform 0.2s ease; display: flex; flex-direction: column; background: var(--blanc, #fff); border-radius: var(--radius-lg, 16px); box-shadow: var(--shadow-lg);">
          <div class="modal-header" style="display:flex;align-items:center;justify-content:space-between;padding:20px 24px;border-bottom:1px solid var(--gris-moyen,#e0e0e0);">
            ...
            <button class="modal-close confirm-cancel" aria-label="Fermer" style="width: 32px; height: 32px; border-radius: 50%; border: none; background: var(--gris-clair, #f5f5f5); cursor: pointer; display: flex; align-items: center; justify-content: center; color: var(--gris-texte, #666); font-size:18px;">&times;</button>
          </div>
          <div class="modal-body" style="padding:24px;overflow-y:auto;max-height:60vh;">
            <p style="color:var(--color-text-secondary,#555);line-height:1.6;margin:0;font-size:14px;">${message}</p>
          </div>
          <div class="modal-footer" style="display:flex;align-items:center;justify-content:flex-end;gap:12px;padding:16px 24px;border-top:1px solid var(--gris-moyen,#e0e0e0);background:var(--gris-background,#fcfcfc);">
      ...
      overlay.innerHTML = `
        <div class="modal" style="max-width: 440px; transform: scale(0.95); transition: transform 0.2s ease; display: flex; flex-direction: column; background: var(--blanc, #fff); border-radius: var(--radius-lg, 16px); box-shadow: var(--shadow-lg);">
          <div class="modal-header" style="display:flex;align-items:center;justify-content:space-between;padding:20px 24px;border-bottom:1px solid var(--gris-moyen,#e0e0e0);">
            ...
            <button class="modal-close alert-ok" aria-label="Fermer" style="width: 32px; height: 32px; border-radius: 50%; border: none; background: var(--gris-clair, #f5f5f5); cursor: pointer; display: flex; align-items: center; justify-content: center; color: var(--gris-texte, #666); font-size:18px;">&times;</button>
          </div>
          <div class="modal-body" style="padding:24px;overflow-y:auto;max-height:60vh;">
            <p style="color:var(--color-text-secondary,#555);line-height:1.6;margin:0;font-size:14px;">${message}</p>
          </div>
  ...
  function showPrompt(title, message, options = {}) {
    ...
    return new Promise((resolve) => {
      ...
      overlay.innerHTML = `
        <div class="modal" style="max-width: 440px; transform: scale(0.95); transition: transform 0.2s ease; display: flex; flex-direction: column; background: var(--blanc, #fff); border-radius: var(--radius-lg, 16px); box-shadow: var(--shadow-lg);">
          <div class="modal-header" style="display:flex;align-items:center;justify-content:space-between;padding:20px 24px;border-bottom:1px solid var(--gris-moyen,#e0e0e0);">
            ...
            <button class="modal-close prompt-cancel" aria-label="Fermer" style="width: 32px; height: 32px; border-radius: 50%; border: none; background: var(--gris-clair, #f5f5f5); cursor: pointer; display: flex; align-items: center; justify-content: center; color: var(--gris-texte, #666); font-size:18px;">&times;</button>
          </div>
          <div class="modal-body" style="padding:24px;overflow-y:auto;max-height:60vh;display:flex;flex-direction:column;gap:10px;">
            <p style="color:var(--color-text-secondary,#555);line-height:1.6;margin:0;font-size:14px;">${message}</p>
            <input type="${inputType}" class="prompt-input" placeholder="${placeholder}" value="${defaultValue}" style="width:100%; padding:10px 14px; border:1px solid var(--gris-moyen,#e0e0e0); border-radius:8px; font-size:14px; outline:none; transition: border-color 0.2s ease;" />
      ...
      inputEl.onblur = () => {
        inputEl.style.borderColor = 'var(--gris-moyen,#e0e0e0)';
